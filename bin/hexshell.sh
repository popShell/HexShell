...
SRC_PATH="$(dirname "$(readlink -f "$0")")/../src"
...
setup_venv() {
    if [ ! -d "$VENV_PATH" ]; then
        ...
        pip install textual watchdog pyyaml rich numpy matplotlib libtmux click
...
main() {
    ...
    tmux send-keys -t "$HEXSHELL_SESSION" "cd $SRC_PATH" C-m
